...
class readexcel(object): 
    ...
    The dict generator assumes that the worksheet is in tabular 
    format with the first "data" row containing the variable names and 
    all subsequent rows containing values. Extracted data is represented 
    fairly logically. By default dates are returned as strings 
    in "yyyy/mm/dd" format or "yyyy/mm/dd hh:mm:ss", as appropriate. 
    However, when specifying date_as_tuple=True, dates will be returned 
    as a (Year, Month, Day, Hour, Min, Second) tuple, for usage with 
    mxDateTime or DateTime. Numbers are returned as either INT or FLOAT, 
    whichever is needed to support the data. Text, booleans, and error 
    codes are also returned as appropriate representations. 
    ...
    def is_data_row(self, sheet, i): 
        ...
        for v in values: 
            if bool(v): 
                return True #+ row full of (valid) False values? return False 
    ...
    def iter_dict(self, sname, date_as_tuple=False): 
        ...
        sheet = self.book.sheet_by_name(sname) 
        # XLRDError # parse first row, set dict keys & first_row_index 
        keys = [] 
...
if __name__ == '__main__':
  afile = os.path.abspath('./SP1000.xls')
  xls = readexcel(afile) 
  for sname in xls.book.sheet_names(): 
      for row in xls.iter_dict(sname): 
          print row 
