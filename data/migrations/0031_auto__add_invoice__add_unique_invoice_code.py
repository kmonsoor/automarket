...
class Migration(SchemaMigration):
    ...
    def forwards(self, orm):
        ...
        db.send_create_signal('data', ['Invoice'])

        # Adding unique constraint on 'Invoice', fields ['code']
        db.create_unique('data_invoice', ['code'])

        # Adding index on 'OrderedItem', fields ['invoice_code']
        db.create_index('data_ordereditem', ['invoice_code'])

        # Adding index on 'OrderedItem', fields ['part_number']
        db.create_index('data_ordereditem', ['part_number'])


    def backwards(self, orm):
        # Removing index on 'OrderedItem', fields ['part_number']
        db.delete_index('data_ordereditem', ['part_number'])

        # Removing index on 'OrderedItem', fields ['invoice_code']
        db.delete_index('data_ordereditem', ['invoice_code'])

        # Removing unique constraint on 'Invoice', fields ['code']
        db.delete_unique('data_invoice', ['code'])

        # Deleting model 'Invoice'
        db.delete_table('data_invoice')
