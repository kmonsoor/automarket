...
class Migration(SchemaMigration):
    ...
    def forwards(self, orm):
        # Removing unique constraint on 'Shipment', fields ['manager', 'code', 'client']
        db.delete_unique('data_shipment', ['manager_id', 'code', 'client_id'])

        # Adding field 'Shipment.number'
        db.add_column('data_shipment', 'number',
                      self.gf('django.db.models.fields.IntegerField')(default=1),
                      keep_default=False)

        # Adding unique constraint on 'Shipment', fields ['code', 'number', 'client']
        db.create_unique('data_shipment', ['code', 'number', 'client_id'])


    def backwards(self, orm):
        # Removing unique constraint on 'Shipment', fields ['code', 'number', 'client']
        db.delete_unique('data_shipment', ['code', 'number', 'client_id'])

        # Deleting field 'Shipment.number'
        db.delete_column('data_shipment', 'number')

        # Adding unique constraint on 'Shipment', fields ['manager', 'code', 'client']
        db.create_unique('data_shipment', ['manager_id', 'code', 'client_id'])
