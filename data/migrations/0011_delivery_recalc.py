...
import logging
logger = logging.getLogger('data.migrations.add_delivery_coef')

class Migration(SchemaMigration):

    def forwards(self, orm):
        def invoice_parse(invoice_code):
            return float(invoice_code.split('#')[-1])

        orders_us = OrderedItem.objects.\
            filter(brandgroup__direction__title__iexact='us')
        notsaved = []
        for order in orders_us:
            if order.invoice_code:
                try:
                    invoice = invoice_parse(order.invoice_code)
                except:
                    notsaved.append(order.id)
                    continue
                if invoice <= float(120):
                    order.delivery_coef = float(13)
                else:
                    order.delivery_coef = float(13.5)
                order.save()
            else:
                notsaved.append(order.id)
        logger.debug("OrderedItem notsaved: %r"%notsaved)
