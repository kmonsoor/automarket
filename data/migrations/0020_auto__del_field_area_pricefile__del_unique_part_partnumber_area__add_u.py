...
class Migration(SchemaMigration):
    ...
    def forwards(self, orm):
        # Removing unique constraint on 'Part', fields ['partnumber', 'area']
        db.delete_unique('data_part', ['partnumber', 'area_id'])

        # Deleting field 'Area.pricefile'
        db.delete_column('data_area', 'pricefile')

        # Adding unique constraint on 'Part', fields ['partnumber', 'brandgroup', 'area']
        db.create_unique('data_part', ['partnumber', 'brandgroup_id', 'area_id'])


    def backwards(self, orm):
        # Removing unique constraint on 'Part', fields ['partnumber', 'brandgroup', 'area']
        db.delete_unique('data_part', ['partnumber', 'brandgroup_id', 'area_id'])

        # Adding field 'Area.pricefile'
        db.add_column('data_area', 'pricefile',
                      self.gf('django.db.models.fields.files.FileField')(max_length=100, null=True, blank=True),
                      keep_default=False)

        # Adding unique constraint on 'Part', fields ['partnumber', 'area']
        db.create_unique('data_part', ['partnumber', 'area_id'])
